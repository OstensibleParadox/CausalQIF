...
/-! # Entropy of a Finite PMF

Base-2 entropy `entropy` via `entropyOf` and `negMulLog2`, plus the pointwise
bound `pmf_le_one`, non-negativity, the equiv-invariance lemma
`entropyOf_equiv_eq`, and the natural-log conversion `entropyOf_mul_log2`.
-/
