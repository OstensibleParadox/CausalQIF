...
theorem semantic_closure_iff_quotient_factorization {E : Type*} [s : Setoid E] (R : E → ℝ) :
    (∀ e1 e2 : E, e1 ≈ e2 → R e1 = R e2) ↔
    (∃ R_bar : Quotient s → ℝ, ∀ e : E, R e = R_bar ⟦e⟧) :=
  factors_through_quotient_iff (R := R)

@[deprecated semantic_closure_iff_quotient_factorization (since := "2026-06-30")]
alias semantic_factorization_iff := semantic_closure_iff_quotient_factorization
