import CausalQIF.DSeparation.TraceSynthesis.OpenTrace.BadColliders
...
theorem openTrace_of_countBadColliders_zero {G : Graph.DAG V} {X Y Z : Finset V} {x y : V}
  ...
  | cons step rest ih =>
      ...
      | directForward hEdge hu hv =>
          have hsrcZ : src ∉ Z := (Finset.mem_sdiff.mp (by
            simpa [Graph.DAG.dSeparationGraphNodes] using hu)).2
      ...
      | directBackward hEdge hu hv =>
          have hnotBad :
              ¬ (arrival = TrailDir.into ∧ Disjoint ({src} ∪ Graph.descendants G src) Z) := by
            intro h
            simp [countBadColliders, isStepBad, h] at hzero
            have hsrcZ : src ∉ Z := by
              intro hsrcMemZ
              exact (Finset.disjoint_left.mp h.2 (by simp)) hsrcMemZ
            have hdescZ : Disjoint (Graph.descendants G src) Z := by
          ...
          have hopen : ¬ directionalTripleBlocked G Z src arrival TrailDir.outOf := by
            cases arrival with
            | into =>
                have hdis : ¬ Disjoint ({src} ∪ Graph.descendants G src) Z := by
                  intro hdis
...
theorem activeRoute_of_countBadColliders_zero {G : Graph.DAG V} {X Y Z : Finset V} {x y : V}
    {arrival : TrailDir} {route : StaticRoute G X Y Z x y}
    (hzero : countBadColliders arrival route = 0) :
...
theorem activeTrail_of_countBadColliders_zero {G : Graph.DAG V} {X Y Z : Finset V} {x y : V}
    {arrival : TrailDir} {route : StaticRoute G X Y Z x y}
    (hzero : countBadColliders arrival route = 0) :
