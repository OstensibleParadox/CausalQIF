import CausalQIF.DSeparation.TraceSynthesis.StaticRoute.Basic
...
noncomputable def ofBackwardReachable {G : Graph.DAG V} {X Y Z : Finset V} {u v : V}
  ...
  G.acyclic.fix (C := fun v => Graph.reachable G u v →
      (∀ n, Graph.reachable G u n → Graph.reachable G n v →
        n ∈ G.dSeparationGraphNodes X Y Z) →
      StaticRoute G X Y Z v u)
...
lemma ofBackwardReachable_eq {G : Graph.DAG V} {X Y Z : Finset V} {u v : V}
    (hreach : Graph.reachable G u v)
    (hnodes :
      ∀ n, Graph.reachable G u n → Graph.reachable G n v →
        n ∈ G.dSeparationGraphNodes X Y Z) :
    ofBackwardReachable hreach hnodes =
      if h : u = v then
        ...
        have h_v_nodes : v ∈ G.dSeparationGraphNodes X Y Z :=
          hnodes v hreach .refl
        StaticRoute.cons (StaticStep.directBackward h_m_v h_v_nodes h_m_nodes)
          (ofBackwardReachable h_u_m
            (fun n h1 h2 => hnodes n h1 (h2.trans (.single h_m_v)))) := by
  unfold ofBackwardReachable
  rw [WellFounded.fix_eq]

/-- Construct a chain of forward directed steps from directed reachability. -/
noncomputable def ofForwardReachable {G : Graph.DAG V} {X Y Z : Finset V} {u v : V}
    (hreach : Graph.reachable G u v)
    (hnodes :
      ∀ n, Graph.reachable G u n → Graph.reachable G n v →
        n ∈ G.dSeparationGraphNodes X Y Z) :
...
lemma ofForwardReachable_eq {G : Graph.DAG V} {X Y Z : Finset V} {u v : V}
    (hreach : Graph.reachable G u v)
    (hnodes :
      ∀ n, Graph.reachable G u n → Graph.reachable G n v →
        n ∈ G.dSeparationGraphNodes X Y Z) :
